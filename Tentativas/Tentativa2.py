...
# Configuração da chave da API diretamente no código
api_key = "AIzaSyCp55HrtDFDBHZvqIeEOelLGcvpraO1X3w"  # Substitua por sua chave de API
...
# Função para remover asteriscos e outros caracteres não necessários
...
def formatar_paragrafos(doc):
    for paragrafo in doc.paragraphs:
        paragrafo.paragraph_format.left_indent = Pt(1.25)  # Recuo de 1,25 cm
        paragrafo.paragraph_format.line_spacing = 1.5  # Espaçamento de 1,5 entre as linhas

# Função para gerar o artigo
def gerar_artigo_abnt(titulo, tema):
...
def salvar_em_pdf(titulo, texto):
    ...
    styles.add(ParagraphStyle(name='Titulo', fontSize=16, spaceAfter=12, alignment=1))  # centralizado
   ...
   # Regex para detectar seções, agora com suporte a conteúdo na mesma linha
    padroes = {
    ...
    for linha in texto.splitlines():
        ...
        for nome_secao, padrao in padroes.items():
            ...
            if match:
                atual = nome_secao
                conteudo[atual] = []
                if match.groups() and match.group(1).strip():  # Se tiver conteúdo na mesma linha
                    conteudo[atual].append(match.group(1).strip())
                mudou_secao = True
                break
    ...
    for secao in ["Resumo", "Abstract", "Palavras-chave", "Introdução", "Revisão de Literatura", "Metodologia", "Resultados e Discussão", "Conclusão", "Referências"]:
        elementos.append(Paragraph(secao, styles['Secao']))
        if secao in conteudo and conteudo[secao]:
            for paragrafo in conteudo[secao]:
                if paragrafo.strip().lower() != secao.lower():  # Evita duplicação do título
    ...
    doc.build(elementos)
    print(f"\n✅ PDF salvo como: {nome_arquivo}, Salvo em {caminho_arquivo}")
...
def salvar_em_docx(titulo, texto):
    ...
    padroes = {
        ...
        "Resultados e Discussão": re.compile(r"^(?:\*+)?\s*\d{0,1}\.?\s*resultados e discussão[:\*]*\s*$", re.IGNORECASE),
        "Conclusão": re.compile(r"^(?:\*+)?\s*\d{0,1}\.?\s*conclusão[:\*]*\s*$", re.IGNORECASE),
        "Referências": re.compile(r"^(?:\*+)?\s*\d{0,1}\.?\s*refer[eê]ncias\s*[:\*]*\s*$", re.IGNORECASE)
    }
    ...
    # Gerar o documento com seções formatadas
    for secao in ["Resumo", "Abstract", "Palavras-chave", "Introdução", "Revisão de Literatura", "Metodologia", "Resultados e Discussão", "Conclusão", "Referências"]:
        doc.add_heading(secao, level=2)
        if secao in conteudo and conteudo[secao]:
            for paragrafo in conteudo[secao]:
                p = doc.add_paragraph(paragrafo)
                p.paragraph_format.left_indent = Pt(28.35)
                run = p.runs[0]
